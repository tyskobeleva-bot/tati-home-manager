...
async def create_cart_from_text(shopping_list_text: str) -> Dict[str, Any]:
    ...
    Uses deterministic orchestration instead of MCP AI tool orchestration.
    """
    try:
    ...
    async with streamablehttp_client(VKUSVILL_MCP_URL) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            for line in lines[:20]:
                ...
                added.append(f"{chosen.get('name') or chosen.get('title') or name} — {qty}")

            if not products:
                return {"ok": False, "text": "Не смогла найти товары для корзины. Пришли список короче и конкретнее."}
    ...
    return {"ok": True, "text": text, "raw": cart, "products": products}
